...
class ActivityLog:
    ...
    def export(self,format="json",**filters):
        ...
        if format=="csv":
            import csv; import io; buf=io.StringIO(); w=csv.writer(buf)
...
app = FastAPI(title="Reversing MCP",lifespan=lifespan)
app.add_middleware(CORSMiddleware,allow_origins=["*"],allow_methods=["*"],allow_headers=["*"])
@app.get("/health")
@app.get("/api/health")
async def health(): return {"status":"ok","server":"Reversing MCP","version":"0.1.0"}
@app.get("/api/logs")
